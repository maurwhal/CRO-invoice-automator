...
(function (global) {
  ...
  async function buildSummaryDocx(data) {
    ...
    var coreXml = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' +
      ...
      '<dc:title>' + esc(data.title || 'Invoice Summary') + '</dc:title><dc:creator>' + esc(data.submittedBy || '') + '</dc:creator>' +
  ...
  // spec: {fields:[{label,value}], chems:[string], submittedBy:string|null, extraParas:[string]}
  ...
  async function fillTemplate(templateBuf, spec) {
    ...
    paras.forEach(function (p) {
      ...
      if (/^Submitted by/.test(text) && spec.submittedBy != null) {
        var ts = p.getElementsByTagNameNS(W, 't');
    ...
    var unmatched = spec.fields.filter(function (f) { return !matched[f.label]; }).map(function (f) { return f.label; });
    return { blob: blob, unmatched: unmatched };
  }
