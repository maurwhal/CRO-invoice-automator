...
(function (global) {
  ...
  function parseGeneric(lines, today) {
    ...
    return {
      ...
      total: money(total), paymentStage: '', lineItems: items,
      project: '', studyDesc: '', started: '', completion: ''
    };
  }

  // Fields on the built-in form: [section, key, label]
  var SUMMARY_FIELDS = [
    ['finance', 'cro', 'CRO / vendor'],
    ['finance', 'invoiceNo', 'Invoice or quote number'],
    ['finance', 'invoiceDate', 'Invoice date'],
    ['finance', 'dueDate', 'Due date'],
    ['finance', 'orderNo', 'PO / order / agreement no.'],
    ['finance', 'total', 'Total amount'],
    ['finance', 'paymentStage', 'Payment stage'],
    ['science', 'studyType', 'Type of study'],
    ['science', 'project', 'Project / study no.'],
    ['science', 'studyDesc', 'Study description'],
    ['science', 'started', 'Date started'],
    ['science', 'completion', 'Expected completion / report']
  ];

  // Labels a user's own template can use. A paragraph that starts with one of
  // these gets the value written after it.
  var TEMPLATE_LABELS = {
    company: 'Company:', cro: 'CRO:', invoiceNo: 'Invoice Number:', invoiceDate: 'Invoice Date:',
    dueDate: 'Due Date:', orderNo: 'PO Number:', total: 'Total Amount:', paymentStage: 'Payment Stage:',
    studyType: 'Type of Study:', project: 'Project Number:', studyDesc: 'Study Description:',
    started: 'Date Started:', completion: 'Completion Date:'
  };
  // Multi-line entries: each line goes under the paragraph that starts with the label
  var TEMPLATE_LISTS = { lineItems: 'Line Items:', materials: 'Test Materials:', notes: 'Notes:' };
  ...
  function parseLines(lines, lab, today, settings) {
    ...
    out.summary = sum;
    out.summaryFilename = safeName((S.docType || 'Invoice Summary') + ' ' + (sum.cro || '').split(/\s+/)[0] + ' ' + (sum.invoiceNo || '')) + '.docx';
  ...
  async function buildSummaryDocx(data) {
    ...
    var body =
      xPara(xRun(data.title || 'Invoice Summary', { b: true, sz: 32, color: '1F3864' }), { after: 40 }) +
    ...
    return packDocx(body, data.title || 'Invoice Summary', data.submittedBy || '');
  }
  ...
  // spec: {fields:[{label,value}], lists:[{label, lines}], chems:[string], submittedBy:string|null, extraParas:[string]}
      ...
      for (var li = 0; li < lists.length; li++) {
        if (text.indexOf(lists[li].label) === 0) {
          ...
          (lists[li].lines || []).forEach(function (line) {
            p.appendChild(makeRun(doc, '', { br: true }));
            p.appendChild(makeRun(doc, line, {}));
          });
  ...
  // A blank template with every label, for people to restyle in Word
  function buildStarterTemplate(title) {
    var L = TEMPLATE_LABELS, line = function (lab) { return xPara(xRun(lab, { b: true })); };
    var head = function (t) { return xPara(xRun(t, { b: true, sz: 24, color: '1F3864' }), { before: 240, after: 80 }); };
    var body =
      xPara(xRun(title || 'Invoice Summary', { b: true, sz: 32, color: '1F3864' }), { after: 120 }) +
      line(L.company) +
      head('Finance / Accounting') +
      ['cro', 'invoiceNo', 'invoiceDate', 'dueDate', 'orderNo', 'total', 'paymentStage'].map(function (k) { return line(L[k]); }).join('') +
      line(TEMPLATE_LISTS.lineItems) +
      head('Science / Operations') +
      ['studyType', 'project', 'studyDesc', 'started', 'completion'].map(function (k) { return line(L[k]); }).join('') +
      line(TEMPLATE_LISTS.materials) + line(TEMPLATE_LISTS.notes) +
      xPara(xRun('Submitted by:'), { before: 360 }) + xPara(xRun('Approved by:'));
    return packDocx(body, title || 'Invoice Summary template', '');
  }
