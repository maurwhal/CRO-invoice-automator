...
(function (global) {
  ...
  function parseTkl(lines, today, S) {
    ...
    // This invoice's share of the study fee, e.g. 35,750 of 71,500 = 50%
    var fee = +find(lines, /TOTAL STUDY FEE\s*:?\s*\$?\s*([\d,]+\.\d{2})/i).replace(/,/g, '');
    var amt = +total.replace(/,/g, '');
    var pct = fee && amt ? Math.round(amt / fee * 100) : 50;
    var payment = {};
    if (phase) payment[phase.toLowerCase()] = pct;
    return {
      summary: {
        invoiceNo: inv, invoiceDate: invDate ? fmtLong(invDate) : '',
        dueDate: invDate ? fmtLong(addDays(invDate, 30)) : '', total: money(total),
        paymentStage: phase ? phase + ' payment' : '', payment: phase ? payment : null,
  ...
  function parseHilltop(lines, today, S) {
    ...
    return {
      summary: { invoiceNo: inv, invoiceDate: ordered, total: money(total), payment: { initiation: 100 } },
  ...
  function parseIivsProposal(lines, today, S) {
    ...
    return {
      summary: {
        ...
        paymentStage: 'Cost proposal (not yet invoiced)', payment: { completion: 100 },
        croPhone: find(lines, /In Vitro Sciences.*?(\(\d{3}\)\s*\d{3}-\d{4})/i),  // letterhead footer
  ...
  function parseGeneric(lines, today) {
    ...
    // CRO phone: from the CRO's own contact lines only. "Phone#" table rows
    // list the bill-to / ship-to numbers, so they are skipped.
    var PHONE = /(\+?1[\s.-]*)?\(?\d{3}\)?[\s.-]*\d{3}[\s.-]*\d{4}(?:\s*(?:x|ext\.?)\s*\d{1,5})?/i;
    var croPhone = '';
    for (var pi = 0; pi < lines.length && !croPhone; pi++) {
      var PL = lines[pi], pm;
      if (/Phone#/i.test(PL)) continue;
      if (/Direct line|Phone\s*:|Tel(?:ephone)?\s*:|Main\s*:/i.test(PL) && (pm = PL.match(PHONE))) croPhone = pm[0];
      else if (/Billing inquiries|Accounts receivable/i.test(PL)) {
        for (var pj = pi + 1; pj <= pi + 3 && pj < lines.length && !croPhone; pj++) {
          if ((pm = lines[pj].match(PHONE)) && !/Phone#/i.test(lines[pj])) croPhone = pm[0];
        }
      }
    }
    croPhone = croPhone.replace(/\s*-\s*/g, '-').replace(/\s+/g, ' ').trim();

    // Payment split stated on the invoice, e.g. "50% due upon initiation"
    var payment = {};
    lines.forEach(function (L) {
      var m = L.match(/(\d{1,3})\s*%\s*(?:due\s*)?(?:upon|at|on)\s*(?:the\s*)?(?:study\s*)?(initiation|start|completion|final report|draft report|report)/i);
      if (m) payment[/^(initiation|start)/i.test(m[2]) ? 'initiation' : /report/i.test(m[2]) ? 'report' : 'completion'] = +m[1];
    });

    return {
      cro: cro, croPhone: croPhone, invoiceNo: invoiceNo, invoiceDate: invoiceDate, dueDate: dueDate, orderNo: orderNo,
      total: money(total), paymentStage: '', payment: payment, lineItems: items,
      project: '', studyDesc: '', started: '', completion: ''
    };
  }

  // "50% Initiation, 50% Completion". Only stages with a percentage are listed.
  var PAY_STAGES = [['initiation', 'Initiation'], ['completion', 'Completion'], ['report', 'Report']];
  function formatPayment(p) {
    p = p || {};
    return PAY_STAGES.filter(function (s) { return +p[s[0]] > 0; })
      .map(function (s) { return (+p[s[0]]) + '% ' + s[1]; }).join(', ');
  }

  // Check boxes on the form; ticked ones are highlighted in the template
  var CATEGORIES = ['Research', 'Safety Assessment', 'Other'];
  var ENDPOINTS = ['Respiratory', 'Repeat Dose/Reproduction', 'Skin Sensitization', 'Genotoxicity',
    'Photoirritation/Photoallergy', 'Environmental', 'Exposure', 'Computational Chemistry'];
  var HIGHLIGHT_ALIASES = { 'Computational Chemistry': ['Computation Chemistry'] };
  ...
  function parseLines(lines, lab, today, settings) {
    ...
    sum.paymentConditions = formatPayment(sum.payment);
    sum.dateSigned = fmtLong(today);
    sum.boardApproval = 'No';
    out.summary = sum;
  ...
  // ── highlighting ticked Category / Endpoint words ──
  // Word often splits a word across runs ("Environmenta" + "l"), so each run is
  // first broken into one run per piece, then the runs covering the word are
  // cut out and highlighted.
  var RPR_AFTER_HIGHLIGHT = ['u', 'effect', 'bdr', 'shd', 'fitText', 'vertAlign', 'rtl', 'cs', 'em', 'lang', 'eastAsianLayout', 'specVanish', 'oMath'];
  function setHighlight(doc, r) {
    var rPr = r.firstElementChild;
    if (!rPr || rPr.localName !== 'rPr') { rPr = wEl(doc, 'rPr'); r.insertBefore(rPr, r.firstChild); }
    Array.prototype.slice.call(rPr.getElementsByTagNameNS(W, 'highlight')).forEach(function (h) { h.parentNode.removeChild(h); });
    var h = wEl(doc, 'highlight'); wAttr(h, 'val', 'yellow');
    var before = null;
    for (var c = rPr.firstElementChild; c && !before; c = c.nextElementSibling) if (RPR_AFTER_HIGHLIGHT.indexOf(c.localName) >= 0) before = c;
    rPr.insertBefore(h, before);
  }
  function directRuns(p) { return Array.prototype.filter.call(p.childNodes, function (c) { return c.nodeType === 1 && c.localName === 'r'; }); }
  function explodeRuns(p) {
    directRuns(p).forEach(function (r) {
      var parts = Array.prototype.filter.call(r.childNodes, function (c) { return c.nodeType === 1 && c.localName !== 'rPr'; });
      if (parts.length < 2) return;
      var rPr = r.firstElementChild && r.firstElementChild.localName === 'rPr' ? r.firstElementChild : null;
      parts.forEach(function (part) {
        var nr = r.cloneNode(false);
        if (rPr) nr.appendChild(rPr.cloneNode(true));
        nr.appendChild(part.cloneNode(true));
  ...
  function runText(r) {
    var c = r.firstElementChild && r.firstElementChild.localName === 'rPr' ? r.firstElementChild.nextElementSibling : r.firstElementChild;
    if (!c) return { kind: '', text: '' };
    if (c.localName === 't') return { kind: 't', text: c.textContent, t: c };
    if (c.localName === 'tab') return { kind: 'tab', text: '\t' };
    return { kind: '', text: '' };
  }
  function highlightWord(doc, p, word) {
    var runs = directRuns(p), info = runs.map(runText), full = '', starts = [];
    info.forEach(function (x) { starts.push(full.length); full += x.text; });
    var re = new RegExp('(^|[\\s\\t])(' + escRe(word).replace(/\//g, '\\s*/\\s*') + ')(?=$|[\\s\\t])', 'ig'), m, hits = [];
    while ((m = re.exec(full))) hits.push([m.index + m[1].length, m.index + m[1].length + m[2].length]);
    hits.reverse().forEach(function (hit) {
      var inside = [];  // runs covering the word, last to first
      for (var i = runs.length - 1; i >= 0; i--) {
        var x = info[i], s = starts[i], e = s + x.text.length;
        if (x.kind !== 't' || e <= hit[0] || s >= hit[1]) continue;
        var a = Math.max(hit[0], s) - s, b = Math.min(hit[1], e) - s, T = x.text, r = runs[i];
        if (b < T.length) { var suf = r.cloneNode(true); runText(suf).t.textContent = T.slice(b); r.parentNode.insertBefore(suf, r.nextSibling); }
        if (a > 0) { var pre = r.cloneNode(true); runText(pre).t.textContent = T.slice(0, a); r.parentNode.insertBefore(pre, r); }
        x.t.textContent = T.slice(a, b);
        x.t.setAttributeNS(XML_NS, 'xml:space', 'preserve');
        inside.unshift({ r: r, t: x.t });
      }
      // Join a word Word had split ("Environmenta" + "l") back into one run
      if (!inside.length) return;
      var first = inside[0];
      first.t.textContent = inside.map(function (y) { return y.t.textContent; }).join('');
      inside.slice(1).forEach(function (y) { y.r.parentNode.removeChild(y.r); });
      setHighlight(doc, first.r);
    });
    return hits.length > 0;
  }
  // Only the top of the form (before Contractor / Invoice / Sub-category lines)
  function highlightOptions(doc, paras, names) {
    var found = [];
    for (var i = 0; i < paras.length; i++) {
      if (/^(Contractor|Invoice|Sub-?\s*category)/i.test(paraText(paras[i]).trim())) break;
      explodeRuns(paras[i]);
      names.forEach(function (n) {
        [n].concat(HIGHLIGHT_ALIASES[n] || []).forEach(function (w) {
          if (highlightWord(doc, paras[i], w) && found.indexOf(n) < 0) found.push(n);
        });
      });
    }
    return found;
  ...
  async function fillTemplate(templateBuf, spec) {
    ...
    var highlighted = spec.highlight && spec.highlight.length ? highlightOptions(doc, paras, spec.highlight) : [];
    paras.forEach(function (p) {
      ...
      for (var i = 0; i < spec.fields.length; i++) {
        ...
        if (text.indexOf(f.label) === 0) {
          if (inherit) {
            // Keep the template's label; add the value in the label's font, not bold.
            // Text the template already has after the label (e.g. a CRO template's
            // fixed "Contractor: Example Labs") is kept, and a trailing "$" takes
            // the amount without a second "$".
            var after = text.slice(f.label.length).trim();
            if (after === '$') { if (f.value) p.appendChild(styledRun(doc, p, f.value.replace(/^\$\s*/, ''))); }
            else if (after) { kept.push(f.label); }
            else if (f.value) p.appendChild(styledRun(doc, p, (/\s$/.test(paraText(p)) ? '' : ' ') + f.value));
          } else {
    ...
    return { blob: blob, unmatched: unmatched, matched: Object.keys(matched), kept: kept, highlighted: highlighted };
  }
